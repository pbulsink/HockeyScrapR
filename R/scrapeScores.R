...
#' @keywords internal
#' @export
readHockeyData <- function(data_dir = "./data/scores/", nhl_year_list = c(1918:2017), wha_year_list = c(1973:1979), 
  playoffs = TRUE, lastPlayoffs = FALSE, ...) {
  ...
  for (year in 1:length(nhl_year_list)) {
    df_nhl <- rbind(df_nhl, utils::read.csv(paste(data_dir, nhl_year_list[year] - 1, nhl_year_list[year], 
      ".csv", sep = ""), stringsAsFactors = FALSE)[2:7])
  }
  if (playoffs) {
...
#' @keywords internal
cleanHockeyData <- function(hockey_data, cleanTeams = TRUE, identifyTies = TRUE, listWinnersLosers = TRUE, 
  removeInternational = TRUE, eloResults = TRUE, ...) {
  teamReplace <- list(list("Alberta Oilers", "Edmonton Oilers"), list("Mighty Ducks of Anaheim", 
    ...
    "Winnipeg Jets"), list("Toronto Toros", "Birmingham Bulls"), list("Ottawa Nationals", "Birmingham Bulls"), 
    list("Quebec Athletic Club/Bulldogs", "Brooklyn Americans"), list("Hamilton Tigers", "Brooklyn Americans"), 
    list("New York Americans", "Brooklyn Americans"), list("Philadelphia Blazers", "Calgary Cowboys"), 
    list("Vancouver Blazers", "Calgary Cowboys"), list("Oakland Seals", "Cleveland Barons"), 
    list("California Golden Seals", "Cleveland Barons"), list("New England Whalers", "Carolina Hurricanes"), 
    list("Hartford Whalers", "Carolina Hurricanes"), list("Chicago Black Hawks", "Chicago Blackhawks"), 
    list("Quebec Nordiques", "Colorado Avalanche"), list("Kansas City Scouts", "New Jersey Devils"), 
    list("Colorado Rockies", "New Jersey Devils"), list("Minnesota North Stars", "Dallas Stars"), 
    list("Detroit Cougars", "Detroit Red Wings"), list("Detroit Falcons", "Detroit Red Wings"), 
    list("Los Angeles Sharks", "Michigan Stags/Baltimore Blades"), list("New York Raiders", 
      "San Diego Mariners"), list("New York Golden Blades/New Jersey Knights", "San Diego Mariners"), 
    list("Pittsburgh Pirates", "Philadelphia Quakers"), list("Toronto Arenas", "Toronto Maple Leafs"), 
    list("Toronto St. Patricks", "Toronto Maple Leafs"), list("Ottawa Senators (historical)", 
      "St. Louis Eagles"))
  ...
  if (identifyTies) {
    ...
    try(hockey_data[hockey_data$OTStatus %in% c("2OT", "3OT", "4OT", "5OT", "6OT", "OT", "SO"), 
      ]$Tie <- TRUE, silent = TRUE)
  }
  
  # Remove games against international teams
  if (removeInternational) {
    hockey_data <- hockey_data[!(hockey_data$Visitor %in% c("Soviet All-Stars", "Czechoslovakia", 
      "Finland")), ]
  }
  ...
  if (cleanTeams) {
    ...
    try(hockey_data[hockey_data$Visitor == "Winnipeg Jets" & hockey_data$Date < as.Date("1997-01-01", 
      format = "%Y-%m-%d"), ]$Visitor <- "Winnipeg Jets (historical)", silent = TRUE)
    try(hockey_data[hockey_data$Home == "Winnipeg Jets" & hockey_data$Date < as.Date("1997-01-01", 
      format = "%Y-%m-%d"), ]$Home <- "Winnipeg Jets (historical)", silent = TRUE)
    try(hockey_data[hockey_data$Visitor == "Ottawa Senators" & hockey_data$Date < as.Date("1935-01-01", 
      format = "%Y-%m-%d"), ]$Visitor <- "Ottawa Senators (historical)", silent = TRUE)
    try(hockey_data[hockey_data$Home == "Ottawa Senators" & hockey_data$Date < as.Date("1935-01-01", 
      format = "%Y-%m-%d"), ]$Home <- "Ottawa Senators (historical)", silent = TRUE)
    ...
    try(hockey_data[hockey_data$Date > as.Date("1976-09-01", format = "%Y-%m-%d") & hockey_data$Visitor == 
      "Minnesota Fighting Saints", ]$Visitor <- "Cleveland Crusaders", silent = TRUE)
    try(hockey_data[hockey_data$Date > as.Date("1976-09-01", format = "%Y-%m-%d") & hockey_data$Home == 
      "Minnesota Fighting Saints", ]$Home <- "Cleveland Crusaders", silent = TRUE)
  ...
  if (listWinnersLosers) {
    hockey_data$Winner <- factor(apply(hockey_data, 1, function(x) ifelse(x[3] > x[5], x[2], 
      x[4])), levels = levels(hockey_data$Home))
    hockey_data$Loser <- factor(apply(hockey_data, 1, function(x) ifelse(x[3] <= x[5], x[2], 
      x[4])), levels = levels(hockey_data$Home))
  }
