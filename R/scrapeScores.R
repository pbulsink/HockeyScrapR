...
#'
#' @param start Starting year to download
#' @param end Last year to download
...
#' @keywords internal
#' @export
readHockeyData<-function(data_dir="./data/scores/", nhl_year_list = c(1918:2017), wha_year_list = c(1973:1979), playoffs = TRUE, lastPlayoffs = FALSE, ...){
    ...
    for (year in 1:length(nhl_year_list)) {
        df_nhl <- rbind(df_nhl, utils::read.csv(paste("./_data/", nhl_year_list[year] - 1, nhl_year_list[year], ".csv", sep = ""))[2:7])
    }
    if (playoffs) {
        for (year in 1:(length(nhl_year_list) - 1)) {
            if (nhl_year_list[year] != 1920){df_nhl <- rbind(df_nhl, utils::read.csv(paste("./_data/", nhl_year_list[year] - 1, nhl_year_list[year], "Playoffs.csv", sep = ""))[2:7])}
        }
        if (lastPlayoffs) {
            df_nhl <- rbind(df_nhl, utils::read.csv(paste("./_data/", nhl_year_list[length(nhl_year_list)] - 1, nhl_year_list[length(nhl_year_list)], "Playoffs.csv", sep = ""))[2:7])
        }
    }
    ...
    if(length(wha_year_list) > 0){
        ...
        for (year in 1:length(wha_year_list)) {
            df_wha <- rbind(df_wha, utils::read.csv(paste("./_data/wha", wha_year_list[year] - 1, wha_year_list[year], ".csv", sep = ""))[2:7])
        }
        if (playoffs) {
            for (year in 1:(length(wha_year_list))) {
                df_wha <- rbind(df_wha, utils::read.csv(paste("./_data/wha", wha_year_list[year] - 1, wha_year_list[year], "Playoffs.csv", sep = ""))[2:7])
            }
        }
...
#' @keywords internal
cleanHockeyData <- function(hockey_data, cleanTeams = TRUE, identifyTies = TRUE, listWinnersLosers = TRUE, removeInternational = TRUE, eloResults=TRUE) {
    teamReplace <- list(list("Alberta Oilers", "Edmonton Oilers"), list("Mighty Ducks of Anaheim", "Anaheim Ducks"), list("Winnipeg Jets (historical)", "Arizona Coyotes"), 
                        list("Phoenix Coyotes", "Arizona Coyotes"), list("Atlanta Flames", "Calgary Flames"), list("Atlanta Thrashers", "Winnipeg Jets"),
                        ...
                        list("Los Angeles Sharks", "Michigan Stags/Baltimore Blades"), list("New York Raiders", "San Diego Mariners"), 
                        list("New York Golden Blades/New Jersey Knights", "San Diego Mariners"), list("Pittsburgh Pirates", "Philadelphia Quakers"), list("Toronto Arenas", "Toronto Maple Leafs"), 
                        list("Toronto St. Patricks", "Toronto Maple Leafs"), list("Ottawa Senators (historical)", "St. Louis Eagles"))
    ...
    hockey_data$OTStatus <- as.factor(hockey_data$OTStatus)
    hockey_data$League <- as.factor(hockey_data$League)
    
    if (identifyTies) {
        hockey_data$Tie <- FALSE
        hockey_data[hockey_data$OTStatus %in% c("2OT", "3OT", "4OT", "5OT", "6OT", "OT", "SO"), ]$Tie <- TRUE
    }
    
    # Remove games against international teams
    if (removeInternational) {
        hockey_data <- hockey_data[!(hockey_data$Visitor %in% c("Soviet All-Stars", "Czechoslovakia","Finland")), ]
    }
    ...
    if (cleanTeams) {
        ...
        levels(hockey_data$Home) <- c(levels(hockey_data$Home), "Winnipeg Jets (historical)")
        levels(hockey_data$Visitor) <- c(levels(hockey_data$Visitor), "Winnipeg Jets (historical)")
        levels(hockey_data$Home) <- c(levels(hockey_data$Home), "Ottawa Senators (historical)")
        levels(hockey_data$Visitor) <- c(levels(hockey_data$Visitor), "Ottawa Senators (historical)")
        
        try(hockey_data[hockey_data$Visitor == "Winnipeg Jets" & hockey_data$Date < as.Date("1997-01-01", format = "%Y-%m-%d"), ]$Visitor <- "Winnipeg Jets (historical)", silent=TRUE)
        try(hockey_data[hockey_data$Home == "Winnipeg Jets" & hockey_data$Date < as.Date("1997-01-01", format = "%Y-%m-%d"), ]$Home <- "Winnipeg Jets (historical)", silent=TRUE)
        try(hockey_data[hockey_data$Visitor == "Ottawa Senators" & hockey_data$Date < as.Date("1935-01-01", format = "%Y-%m-%d"), ]$Visitor <- "Ottawa Senators (historical)", silent=TRUE)
        try(hockey_data[hockey_data$Home == "Ottawa Senators" & hockey_data$Date < as.Date("1935-01-01", format = "%Y-%m-%d"), ]$Home <- "Ottawa Senators (historical)", silent=TRUE)
        ...
        try(hockey_data[hockey_data$Date > as.Date("1976-09-01", format = "%Y-%m-%d") & hockey_data$Visitor == "Minnesota Fighting Saints", ]$Visitor <- "Cleveland Crusaders", silent=TRUE)
        try(hockey_data[hockey_data$Date > as.Date("1976-09-01", format = "%Y-%m-%d") & hockey_data$Home == "Minnesota Fighting Saints", ]$Home <- "Cleveland Crusaders", silent=TRUE)
        
        hockey_data$Visitor <- droplevels(hockey_data$Visitor)
        hockey_data$Home <- droplevels(hockey_data$Home)
    }
    
    if (listWinnersLosers) {
        hockey_data$Winner <- as.factor(apply(hockey_data, 1, function(x) ifelse(x[3] > x[5], x[2], x[4])))
        hockey_data$Loser <- as.factor(apply(hockey_data, 1, function(x) ifelse(x[3] <= x[5], x[2], x[4])))
    }
