...
scrapePlInfo.HR <- function(url) {
  ...
  if (is.null(nrow(meta_draft)) || nrow(meta_draft) > 0) {
    ...
    names(meta_draft) <- c("DraftTeam", "DraftRound", "DraftOverall", "DraftYear", rep(c("ReDraftTeam",
      "ReDraftRound", "ReDraftOverall", "ReDraftYear"), times = ((length(meta_draft) - 4)/4)))
...
flattenPlayerTables.HR <- function(tables) {
  ...
  if ("stats_misc_nhl" %in% names(tables)) {
    ...
    stmisc[, 7:12] <- NULL
    stmisc <- stmisc[, names(stmisc) %in% c("Season", "Age", "Team", "Lg", "GC", "G", "A", "PTS",
      "GC.1", "OPS", "DPS", "PS")]
...
getPlayerStats.HR <- function(player_list, sleep = 30, progress=TRUE) {
  ...
  while (player < nrow(plist)) {
    ...
    if (is.null(scrape)) {
      ...
      tables <- flattenPlayerTables.HR(scrape[[1]])
      tables$Name <- pname
...
#' @export
#' @keywords internal
scrapeByAlphabet.HR <- function(player_list, letters_to_scrape = letters, long_sleep = 120, combine = TRUE,
  ...
  if (combine == TRUE) {
    return(combinePlayerDataFrames.HR(directory, ...))
  }
  return(TRUE)
}
...
#' @export
#' @keywords internal
combinePlayerDataFrames.HR <- function(directory = "./data/players/", return_data_frame = TRUE, ...) {
...
processPlayerData.HR <- function(player_data, drop_awards = TRUE, ...) {
  players <- player_data[[1]]
  goalies <- player_data[[2]]
  meta <- player_data[[3]]

  # Undo factors
  numeric_columns <- c("Age", "GP", "G", "A", "PTS", "+/-", "PIM", "EV.Goals", "PP.Goals", "SH.Goals",
    "GW", "EV.Assists", "PP.Assists", "SH.Assists", "S", "S%", "TOI", "GC", "Adj.G", "Adj.A",
    "Adj.PTS", "Adj.GC", "TSA", "OPS", "DPS", "PS", "FOW", "FOL", "FO%", "HIT", "BLK", "TK",
    "GV", "CF", "CA", "CF%", "CF%rel", "FF", "FA", "FF%", "FF%rel", "oiSH%", "oiSV%", "PDO",
    "oZS%", "dZS%", "GS", "W", "L", "T/O", "GA", "SA", "SV", "SV%", "GAA", "SO", "MIN", "QS",
    "QS%", "RBS", "GA%-", "GSAA", "GPS")
  ...
  toi[toi == ""] <- "0:0"
  toi[is.na(toi)] <- "0:0"
  players$ATOI <- unlist(lapply(toi, function(x) as.numeric(unlist(strsplit(x, ":")))[1] + as.numeric(unlist(strsplit(x,
    ":"))[2])/60))
  toi <- goalies$ATOI
  toi[toi == ""] <- "0:0"
  toi[is.na(toi)] <- "0:0"
  goalies$ATOI <- unlist(lapply(toi, function(x) as.numeric(unlist(strsplit(x, ":")))[1] + as.numeric(unlist(strsplit(x,
    ":"))[2])/60))
  ...
  imp[imp == ""] <- "0-0"
  imp[is.na(imp)] <- "0-0"
  meta$HeightImp <- unlist(lapply(imp, function(x) as.numeric(unlist(strsplit(x, "-")))[1] * 12 +
  ...
  mnumeric <- c("HeightImp", "WeightImp", "HeightMetric", "WeightMetric", "DraftRound", "DraftOverall",
    "DraftYear", "ReDraftRound", "ReDraftOverall", "ReDraftYear", "ActiveStart", "ActiveEnd")
...
#'
#' @examples
#' \dontrun{scrapePlayers.HR()}
#' \dontrun{scrapePlayers.HR(sleep=15, long_sleep=180)}
...
#'
#' @examples
#' \dontrun{updatePlayers.HR()}
#' \dontrun{updatePlayers.HR(data_dir = './data/', years_back=2}
updatePlayers.HR <- function(player_data, data_dir = "./data/players/", years_back = 1, player_list = NULL,
  ...) {
  ...
  p <- scrapeByAlphabet.HR(player_list = update_players, directory = data_dir, ...)
  p_player <- p[[1]]
  p_goalie <- p[[2]]
  p_meta <- p[[3]]
