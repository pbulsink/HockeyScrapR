teamURLList <- data.frame(URL = c("http://www2.dailyfaceoff.com/teams/lines/13/anaheim-ducks/",
                                  ...
                                  "http://www2.dailyfaceoff.com/teams/lines/14/winnipeg-jets"), Team = c("Anaheim Ducks",
...
getCurrentRosters <- function(sleep = 30, teamUrlList = teamURLList) {
  ...
  for (i in 1:nrow(teamUrlList)) {
    ...
    dt <- as.character(as.Date(gsub("\\.", "", stringr::str_match(htmlpage, pattern)[1,
      2]), format = "%b%t%e,%t%Y"))
